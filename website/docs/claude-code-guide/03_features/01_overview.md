...
- ユーザー設定はすべてのプロジェクトに適用される個人のグローバル設定ファイルです。
...
- ローカルのみで有効な設定。Claude Code起動時に `.gitignore` に追加されますが、必要があれば除外対象に追加してください。
...
| `hooks` | ツール実行の前後に実行するカスタムコマンドを設定します。[フック/カスタムコマンド](./04_hooks.md)を参照 | `{"PreToolUse": {"Bash": "echo 'Running command...'"}}` |
...
メモリファイル(`CLAUDE.md`)は、スタイルガイドラインやワークフローの一般的なコマンドなど、セッション間でユーザーの指示を記憶することができます。

**ユーザーメモリ**
- 場所: `~/.claude/CLAUDE.md`
- ユーザー設定はすべてのプロジェクトに適用される個人のグローバル設定ファイルです。コードスタイリング設定、個人ツールのショートカットなどの記憶に使えます。
...
**プロジェクトメモリ(ローカル)**
- 場所: `.claude/settings.local.json`
- ローカルのみで有効な設定。個人のプロジェクト固有設定に使えますが、チェックイン対象にならないように `.gitignore`に追加しておく必要があります。ただしCLAUDE.mdにはインポート機構が使えるため、個人設定はプロジェクト外で管理しインポートして利用することが推奨されています。
...
この機構を利用してユーザーのホームディレクトリからCLAUDE.local.md相当のファイルを読み込む方法が現在は推奨されています。
```md
# 個人設定
...
```
インポートされたファイルは追加ファイルを再帰的にインポートでき、最大深度は5ホップです。/memoryコマンドを実行することで、どのメモリファイルが読み込まれているかを確認できます。
```
> /memory
```
...
> /init
```
プロジェクトが進んだ状態で使用すると、予期せぬ破壊的な変更を起こす可能性があるため注意が必要です。
...
### モデル
Amazon Bedrockを利用している場合は[セットアップ](../02_prerequisites/02_setup.md)で指定したモデルが表示されています。

プライマリモデル、小型/高速モデルともに最新のモデルを指定して利用したい場合は設定を変更して利用してください。ただし、USリージョンと東京を含むAPACリージョンではモデルの提供状況に違いがあるため注意が必要です。
