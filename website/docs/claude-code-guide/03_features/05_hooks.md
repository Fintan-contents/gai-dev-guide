...
Claude Codeで長時間のタスクを実行している間、別の作業に集中していると、いつタスクが完了したのか、いつユーザの入力が必要になったのか気づきにくいという問題があります。CCNotifyはこの問題を解決するデスクトップ通知フックです。タスクの完了時やユーザ入力が必要になったタイミングで通知を送信し、ワン押下でVS Codeに戻ることができます。さらに、タスクの実行時間も表示されるため、作業の効率性を把握することも可能です。
